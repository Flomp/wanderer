...
export async function lists_show(id: string, handle?: string, f: (url: RequestInfo | URL, config?: RequestInit) => Promise<Response> = fetch) {
    ...
    const r = await f(`/api/v1/list/${id}?` + new URLSearchParams({
        expand: "author,trails,trails.author,trails.category,trails.subcategory,trails.subcategory.category,trails.tags,trails.trail_assets_via_trail.asset,list_share_via_list.actor",
        ...(handle ? { handle } : {})
    }), {
        method: 'GET',
    })
