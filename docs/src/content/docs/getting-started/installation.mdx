...
If you run into issues regarding vitest, please ensure it is properly installed. Ohterwise you can get it with
```bash
npm i -s vitest
```

Further issue which can appear is import error for PUBLIC_VALHALLA_URL. You can export it using
```bash
